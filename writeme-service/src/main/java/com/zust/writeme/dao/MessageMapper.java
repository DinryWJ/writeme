...
public interface MessageMapper extends MyMapper<Message> {
    @Select("SELECT * FROM message AS m WHERE m.status <= 1 AND ( to_user_id = #{userId} AND from_user_id = #{guestId}) OR ( to_user_id = #{guestId} AND from_user_id = #{userId}) ORDER BY create_time ASC")
    @Results({
    ...
    List<Message> getFromUserMessageList(@Param("fromUserId") int fromUserId, @Param("status") int status);
    ...
    List<Message> getToUserMessageList(@Param("toUserId")int toUserId, @Param("status") int status);
}
